...
<div>
<form action="{{ $curriculum ? route('curriculums_edit', ['id' => $curriculum->id]) : route('curriculums_create') }}" method="post" enctype="multipart/form-data">
        @csrf
        @if($curriculum)
            @method('PUT')
        @endif
        ...
        <div class="submit-btn">
            <button class="submit" type="submit">{{ $curriculum ? '更新' : '登録' }}</button>
        </div>
    </form>
</div>
